...
event-abandoned-corpse |	방치된 시체 발견 |	관리국이 미처 수거하지 못했거나 화물 사이에 낀 신선한(?) 사체를 발견합니다.	전투 없이 강력한 미니언 소체 혹은 시체 폭발용 재료 1~2구 확정 획득.
event-voice-recorder |	정비사의 기록기 |	보스 '기계 골렘'을 수리하던 정비사의 긴박한 유언이나 음성 일기를 듣습니다.	보스 약점 파악: 보스전 돌입 시 적중률 +15% 혹은 특정 부위 파괴 기믹 힌트 획득.
event-map-scan | 터미널 안내도 스캔 | 노후화된 터미널 지도를 해킹하거나 복구하여 전체 구조를 파악합니다.	아직 방문하지 않은 타일의 이벤트 종류 및 위험도(몬스터 그룹) 노출.
event-conveyor-control	컨베이어 벨트 가동	멈춰있던 대형 벨트를 강제로 작동시켜 물류 궤도를 바꿉니다.	지름길 개방: 복잡한 경로를 건너뛰고 특정 구역으로 즉시 이동 가능.
event-power-panel	과부하된 배전반	노출된 전선을 조작하여 구역 내 전력을 일시적으로 폭주시키거나 차단합니다.	인근 보안 구역(automated-security) 몬스터들의 1턴간 스턴 혹은 방어력 감소.
event-terminal-lore	하역장의 진실	고철 틈새에서 발견된 노동자의 일기나 기밀 문서를 통해 이곳의 역사를 읽습니다.	세계관 확장: 하역장이 오염된 이유와 '지하 4층'으로 이어지는 불길한 복선 확인.
