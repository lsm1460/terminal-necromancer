...
- 골렘, 기사의 부활 장소 추가 필요.. 추가 npc...전 레지스탕스 일원 닥터 핀치.. 첫 보스 클리어 시 지하 3층에 레지스탕스 본부가 있다는 이벤트
- 상점(아이템 구입, 판매)
...
- 보스 이벤트 후 다크나이트 획득
- 데스나이트.. 강화를..기획을.. 다시 생각해보자.. 전장에서 엘리베이터와 함께 npc로 등장시킬까..
- 시체 회수 & 시체가 월드에 있는데 다시 죽을 경우 이전 시체 삭제

- 전투 중 현재 상태 보기 기능 추가
- look에 enquire 도입하기.. -> 진행 가능 방향으로 보이는 것, 인벤토리, 몬스터, npc -> 대화로 연결 confirm?
  - 군단 상태 표기 시 orderWeight가 작은 순으로 표기
  - printMinion에 미니언의 보유 스킬 표기
  - 인벤토리 내 아이템을 선택했을 때, 어픽스를 가지고 있다면 해당 어픽스의 설명문도 출력
...
- epic: affix 보유
  ...
  - 금기: 다크나이트를 리치로 전직시키고 스킬 (시체 폭발, 뼈 감옥)을 해금함
