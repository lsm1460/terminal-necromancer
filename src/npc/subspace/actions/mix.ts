...
export const handleMix = async (context: GameContext) => {
  ...
  if (!SubspaceService.getMixableGroups(necromancer)) {
  ...
  const selectedIds = await Terminal.multiselect(
    ...
    necromancer.skeleton.map((sk) => ({ name: sk.id, message: `${sk.name} (HP: ${sk.hp}/${sk.maxHp})` })),
    { maxChoices: 2 }
  )

  if (selectedIds.length < 2) return

  const selected = necromancer.skeleton.filter((sk) => selectedIds.includes(sk.id)).sort((a, b) => b.maxHp - a.maxHp)
  ...
  if (
    ...
    if (Math.random() < chance) {
      selected.forEach((sk) => necromancer.removeMinion(sk.id))
      ...
      necromancer.addSkeleton(newSk)
      Terminal.log(getMsg('success'))
    } else {
      necromancer.removeMinion(selected[1].id) // 실패 시 재료 하나 소멸
