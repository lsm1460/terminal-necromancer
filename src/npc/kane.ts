...
async function handleJoin(player: Player, npc: NPC, context: GameContext) {
  ...
  if (!jaxIsAlive) {
    ...
    dialogues.push(
      ...
      '케인: "그 녀석은 잘 있나? 워낙 발이 빨라 걱정은 안 한다만, 사신의 마력이 짙게 깔린 이 구역은 잭스에게도 버거운 곳이었을 텐데."'
    );
  }

  // 2. 사신의 통치에 대한 폭로 및 제안
  dialogues.push(
    '케인: "사신은 이곳을 정화한다는 명목으로 환생의 흐름을 막고 있다. 마땅히 빛으로 돌아가야 할 영혼들을 자신의 정원을 가꾸는 노예로 부리고 있단 말이다."',
    '케인: "비록 죄를 짓고 이곳에 왔을지라도, 영원히 부당한 노역에 시달려야 할 영혼은 없어. 그건 공정이 아니라 그저 사신의 탐욕일 뿐이지."',
    '케인: "이봐, 신참. 사신이 시키는 대로 비굴하게 고개를 숙이며 영혼을 갉아먹힐 건가? 아니면 우리와 함께 이 뒤틀린 체계를 무너뜨릴 건가?"',
    '케인: "선택은 네놈의 몫이다. 하지만 기억해라. 사신의 개로 살다 버려지는 것보다, 인간으로서 저항하다 사라지는 것이 훨씬 가치 있다는 걸."'
  );
