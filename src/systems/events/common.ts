...
export const commonHandlers: Record<string, EventHandler> = {
  ...
  'heal-once': async (tile, player, context) => {
    ...
    Terminal.log(`\x1b[90m(사신의 서늘한 기운이 닿지 않는, 누군가 의도적으로 숨겨둔 듯한 따스한 공간입니다)\x1b[0m`)
    ...
    if (!proceed) {
      Terminal.log(' > 당신은 이 소중한 온기를 나중을 위해 아껴두기로 합니다.')
      return
    }
    ...
    Terminal.log(`\x1b[90m(기운을 다한 꽃들이 투명하게 흩어지며, 다시 차가운 터미널의 공기가 돌아옵니다.)\x1b[0m\n`)
