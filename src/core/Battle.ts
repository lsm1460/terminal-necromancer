...
export class Battle {
  ...
  static attack(player: Player, target: BattleTarget, context: GameContext): boolean {
    ...
    // 1. 주 타겟 데미지 처리
    if ('id' in target && npcs.getNPC(target.id as string)) {
      // 대상이 NPC인 경우
      const result = npcs.takeDamage(target.id as string, p.atk);
    ...
    this.executeGroupCounter(player, target, isTargetDead, context);
   ...
   */
  private static executeGroupCounter(player: Player, primaryTarget: BattleTarget, isPrimaryDead: boolean, context: GameContext) {
    const tile = context.map.getTile(player.pos.x, player.pos.y);
    const enemies: BattleTarget[] = [];

    // 반격 리스트 구성
    if (!isPrimaryDead) enemies.push(primaryTarget);
    
    // 타일에 있는 다른 몬스터 추가
    if (tile.currentMonster && tile.currentMonster !== primaryTarget) {
      enemies.push(tile.currentMonster);
    }

    // 타일에 있는 다른 '적대적' NPC 추가
    (tile.npcIds || []).forEach(id => {
      const npc = context.npcs.getNPC(id);
      if (npc && npc.isAlive && context.npcs.isHostile(id) && npc !== primaryTarget) {
        enemies.push(npc);
      }
    });
    ...
    for (const enemy of enemies) {
      ...
      const counterDmg = Math.max(enemy.atk - player.computed.def, 1);
