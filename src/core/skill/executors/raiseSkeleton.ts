...
export const raiseSkeleton: ExecuteSkill = async (player, context) => {
  ...
  const makeSkeleton = (corpse: Corpse, isMultiple?: boolean) => {
    ...
    const skeleton: BattleTarget = {
      ...
      maxHp: Math.floor(corpse.maxHp * 0.5 * m * s.hp),
      hp: Math.floor(corpse.maxHp * 0.5 * m * s.hp),
      atk: Math.floor(corpse.atk * 0.8 * m * s.atk),
      def: Math.floor(corpse.def * 0.5 * m * s.def),
      agi: Math.floor(corpse.agi * 0.5 * m * s.agi),
